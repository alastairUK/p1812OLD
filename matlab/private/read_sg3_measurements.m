function sg3db=read_sg3_measurements(filename,fileformat)
%
...
%               = 'fryderyk_csv' (implemented)
%               = 'cvs'          (tbi)
%               = 'xml'          (tbi)
%
%  Output variable is a struct sg3db containing the following fields
%  x               - vector of path profile distances
%  h_gamsl         - vector of path profile heights
%  frequency       - vector of frequencies
%  hTx             - vector of Tx antenna heights above ground in m
%  hRx             - vector of Rx antenna heights above ground in m
%  etc
%
...
% Revision History:
% Date            Revision
% 07JUL2016       Included reading of other parameters (Latitude,
%                 Longitude, DN, N0, etc. necessary for P.1812)
...
switch fileformat
    case 'Fryderyk_csv'
        ...
        while(1)
            ...
            % remove trailing ';' that some versions of .csv may have
            readLine=regexprep(readLine,';','');
            
            dummy=regexp(readLine,',','split');
            
            if strcmp1(dummy{1},'Tx LAT')
                sg3db.TxLat=str2double(dummy{2});
            end            
            
            if strcmp1(dummy{1},'Tx LON')
                sg3db.TxLon=str2double(dummy{2});
            end  

            if strcmp1(dummy{1},'Rx LAT')
                sg3db.RxLat=str2double(dummy{2});
            end            
            
            if strcmp1(dummy{1},'Rx LON')
                sg3db.RxLon=str2double(dummy{2});
            end  
            ...
            if strcmp1(dummy{1},'Average annual values DN (N-units/km):')
                sg3db.DN=str2double(dummy{2});

            end              
            
            
            if strcmp1(dummy{1},'Average annual sea-level surface refractivity No (N-units):')
                sg3db.N0=str2double(dummy{2});
            end             
